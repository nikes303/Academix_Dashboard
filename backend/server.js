    ...
    try {
        ...
        const getUpcomingTask = new Promise((resolve, reject) => {
            const sql = `
                SELECT name, dueDate 
                FROM tasks 
                WHERE isCompleted = 0 AND dueDate >= date('now') 
                ORDER BY 
                    dueDate ASC, 
                    CASE priority 
                        WHEN 'high' THEN 1 
        ...
        const [pendingCount, attendancePercent, nextEvent] = await Promise.all([getPendingAssignments, getOverallAttendance, getUpcomingTask]);
        res.json({
            message: "success",
            data: {
                attendance: {
                    overall: `${attendancePercent}% Overall`,
                    text: "Based on last recorded day."
                },
                assignments: {
                    pending: `${pendingCount} Pending`,
                    text: "Track assignment submissions."
                },
                events: {
                    title: "Next Due",
                    text: nextEvent
...
app.put('/api/assignments/:id/complete', (req, res) => {
    const {
        id
    } = req.params;
    const sql = `UPDATE assignments SET status = 'Completed' WHERE id = ?`;
...
app.post('/api/students', (req, res) => {
    ...
    if (!name || !usn) {
        return res.status(400).json({
            "error": "Student Name and USN are required."
        });
    }
    ...
    db.run(sql, [name, usn, branch, section, test1, test2, quiz, assignment], function(err) {
        if (err) {
            return res.status(409).json({
                "error": `A student with USN ${usn} already exists.`
            });
        }
        res.status(201).json({
            "message": "success",
...
app.get('/api/class-list', (req, res) => {
    ...
    if (!branch || !section) {
        return res.status(400).json({
            "error": "Branch and Section are required."
        });
    }
            ...
            const result = await new Promise((resolve, reject) => {
                ...
                db.get(sql, [section], (err, row) => {
                    ...
                    if (row.count === 0) {
                        ...
                        resolve({
                            score: parseFloat(total_score.toFixed(2)),
                            hasData: true
                        });
                    }
                });
...
app.post('/api/notes', (req, res) => {
    ...
    if (!section || !note) {
        return res.status(400).json({
            "error": "Section and note text are required."
        });
    }
